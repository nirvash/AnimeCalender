...
describe('addDays28h 境界条件', () => {
  ...
  it.each(cases)('%s n=0, n=1, n=-1', (_desc, input: string, expected: readonly string[]) => {
    ...
    expect(getJSTDate(addDays28h(base28h, 0))).toBe(expected[0]);
    expect(getJSTDate(addDays28h(base28h, 1))).toBe(expected[1]);
    expect(getJSTDate(addDays28h(base28h, -1))).toBe(expected[2]);
  });
});

describe('isSameDay28h 境界条件', () => {
